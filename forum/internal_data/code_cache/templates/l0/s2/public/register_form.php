<?php
// FROM HASH: efc41251bffe6690713c0de76d495760
	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['providers'])) {
			foreach ($__vars['providers'] AS $__vars['provider']) {
				if ((!$__vars['xf']['options']['blacktea_steamauth_force_registration']) OR ($__templater->method($__vars['provider'], 'getValue', array('provider_id', )) == 'steam')) {
					$__compilerTemp1 .= '
...
';
	if (!$__vars['xf']['options']['blacktea_steamauth_force_registration']) {
		$__finalCompiled .= '
';
		$__compilerTemp2 = '';
